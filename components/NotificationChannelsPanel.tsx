...
/** Couleurs Turnaicash — alignées sur le thème (primary) du dashboard */
const CHANNEL_ACCENT = 'border-primary/40 ring-1 ring-primary/15';
const BTN_PRIMARY = 'bg-primary hover:bg-primary/90 text-primary-foreground';
const BTN_OUTLINE = 'border border-primary text-primary dark:text-primary';
const ICON_BRAND = 'text-primary';
const SELECT_CLASS =
  'w-32 shrink-0 rounded-xl border border-slate-300 bg-white px-3 py-3 text-sm font-medium text-slate-900 outline-none focus:ring-2 focus:ring-primary focus:border-transparent dark:border-slate-500 dark:bg-slate-700 dark:text-white';
const INPUT_CLASS =
  'min-w-0 flex-1 rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-900 placeholder:text-slate-400 outline-none focus:ring-2 focus:ring-primary focus:border-transparent dark:border-slate-500 dark:bg-slate-700 dark:text-white dark:placeholder:text-slate-400';
const INPUT_FULL_CLASS =
  'w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-900 placeholder:text-slate-400 outline-none focus:ring-2 focus:ring-primary focus:border-transparent dark:border-slate-500 dark:bg-slate-700 dark:text-white dark:placeholder:text-slate-400';

export const DISMISS_COUNT_KEY = 'turnaicashNotificationPromptDismissCount_v2';
...
const CHANNEL_SESSION_KEY = 'turnaicash_channel_panel_state';
const CHANNEL_SESSION_TS = 'turnaicash_channel_panel_ts';
const CHANNEL_SESSION_TTL_MS = 5 * 60 * 1000;
    ...
    <ol className="mt-3 space-y-2">
      {steps.map((step, index) => (
        <li key={step} className="flex gap-3 text-xs leading-5 text-slate-600 dark:text-slate-300">
          <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-primary/15 text-[11px] font-bold text-primary dark:bg-primary/25">
            {index + 1}
          </span>
          <span>{step}</span>
        </li>
...
export default function NotificationChannelsPanel({
  ...
  const applyChannelState = useCallback((state: CachedChannelState) => {
    setWhatsappVerified(state.whatsappVerified);
    setTelegramVerified(state.telegramVerified);
    setSmsVerified(state.smsVerified);
    setShowWhatsapp(state.showWhatsapp);
    setShowTelegram(state.showTelegram);
    setShowSms(state.showSms);
    setCountryCode(state.countryCode);
    setSmsPhone(state.smsPhone);
    setWhatsappPhone(state.whatsappPhone);
  ...
  const openTelegramBot = async (link?: string) => {
    ...
    setSuccess('Bot Turnaicash ouvert. Appuyez sur Démarrer / Start, puis revenez ici.');
  };
  ...
  const handleTelegramSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (message === 'USER_MUST_START_BOT' && botLink) {
        ...
        setError('Appuyez sur Start dans le bot Turnaicash, puis réessayez.');
      } else {
        setError('Démarrez le bot Turnaicash (Start), puis réessayez.');
      }
    } finally {
      setLoading(false);
  ...
  return (
    <div>
      {showHeader && (
        <div className="mb-4">
          <div className="mb-3 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary/15 dark:bg-primary/25">
          ...
          >
            ...
            <StepList
              steps={[
                ...
                "Le bot Turnaicash s'ouvre automatiquement — appuyez sur Démarrer / Start.",
          ...
          >
            ...
            <StepList
              steps={[
                ...
                'Vos alertes Turnaicash arriveront ensuite par SMS.',
              ]}
            />
      ...
      {success && (
        <p className="mt-3 rounded-xl bg-primary/10 px-4 py-3 text-sm text-primary dark:bg-primary/20">
          {success}
        </p>
      )}
